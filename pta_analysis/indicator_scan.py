...
def analyze_period(df):
    ...
    return {
        'datetime': last['datetime'].strftime('%H:%M'),
...
for label, code in periods:
    ...
    kj = r['kdj']
    print("")
    print("【%s】%s 价格=%.0f (%d根)" % (label, r['datetime'], r['close'], r['bars']))
    print("  MACD: DIF=%.4f  DEA=%.4f  MACD=%.4f [%s]" % (m['dif'], m['dea'], m['macd'], m['state']))
    print("  KDJ:  K=%.1f    D=%.1f    J=%.1f" % (kj['K'], kj['D'], kj['J']))
    for i, a in enumerate(r['areas']):
        s = "红" if a['sign'] == 1 else "绿"
        print("  MACD面积%d: %s 面积=%.4f %d根" % (i+1, s, a['area'], a['bars']))

with open('indicator_results.json', 'w', encoding='utf-8') as f:
    json.dump(results, f, ensure_ascii=False, indent=2)
print("")
print("Saved to indicator_results.json")
