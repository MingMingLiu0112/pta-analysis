...
PTA 缠论线段检测 v7 - 简化版
规则（用户确认版）：
- 常规情形：下行笔.low < 前一上行笔.high → 笔破坏 → 线段在反向笔起点结束
- 非常规情形：缺口未封闭 → 等后续底分型两步确认
- 线段终点 = 反向笔的起点（gap封闭即确认）
"""
...
class BiBar:
    def __init__(self, bi_idx, dt, direction, start_p, end_p):
        ...
        self.start = start_p  # 起点（向上笔=首bar.low, 向下笔=首bar.high）
        self.end = end_p      # 终点（向上笔=末bar.high, 向下笔=末bar.low）
    ...
    while i < n - 1:
        ...
        if cur.direction == 'up' and next_bi.direction == 'down':
            ...
            if prev_up_idx is not None:
                ...
                prev_up_low = bi_bars[prev_up_idx].low   # 前一上行笔的低点
                gap_closed = next_bi.low < prev_up_high   # 常规情形

                print(f"\n下行笔 bi{next_idx+1}: low={next_bi.low:.0f} vs "
                      f"前一up笔 bi{prev_up_idx+1} high={prev_up_high:.0f} low={prev_up_low:.0f} | "
                      f"{'✓常规' if gap_closed else '✗非常规'}")

                if gap_closed:
                    # 常规情形：线段在反向笔起点结束
                    # 线段: i ~ next_idx (终点=next_bi的起点)
                    results.append((i, next_idx, 'up', next_bi.start))
                    print(f"  → 常规确认! 上行段 bi{i+1}~bi{next_idx+1} 终点={next_bi.start:.0f}")
                    i = next_idx
                    continue
            ...
            if prev_dn_idx is not None:
                ...
                prev_dn_high = bi_bars[prev_dn_idx].high
                gap_closed = next_bi.high > prev_dn_low  # 下行段中反向看high

                print(f"\n上行笔 bi{next_idx+1}: high={next_bi.high:.0f} vs "
                      f"前一dn笔 bi{prev_dn_idx+1} low={prev_dn_low:.0f} | "
                      f"{'✓常规' if gap_closed else '✗非常规'}")

                if gap_closed:
                    results.append((i, next_idx, 'down', next_bi.start))
                    print(f"  → 常规确认! 下行段 bi{i+1}~bi{next_idx+1} 终点={next_bi.start:.0f}")
                    i = next_idx
                    continue
